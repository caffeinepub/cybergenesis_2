...
export const debugGPUContext = (gl: any) => {
  ...
  try {
    ...
    console.log(
      "[GPU Diagnostics] ✅ Using WebGLRenderer - WebGPU is DISABLED",
    );
  } catch (error) {
    console.error(
...
export const detectOverlappingRenderers = () => {
  ...
  console.log(
    `[Renderer Detection] 🔍 Found ${canvases.length} canvas elements`,
  );

  if (canvases.length > 1) {
    console.warn(
      "[Renderer Detection] ⚠️ Multiple canvas elements detected - potential context conflict",
    );
  }
