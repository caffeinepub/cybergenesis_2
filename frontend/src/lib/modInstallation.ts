...
export function installMod(
  ...
  // Mark modifier as applied in fakeCbr inventory
...
export function uninstallMod(instanceId: number, landId: string): boolean {
  ...
  try {
    const raw = localStorage.getItem("fake_modifier_inventory");
    if (raw) {
      const all = JSON.parse(raw) as Array<{ instanceId: number; appliedToLand?: string }>;
      const patched = all.map((m) =>
        m.instanceId === instanceId ? { ...m, appliedToLand: undefined } : m,
      );
      localStorage.setItem("fake_modifier_inventory", JSON.stringify(patched));
    }
  } catch {
    /* ignore */
  }
 ...
 * Multiplies stored anchor coords by landScale (land renders at [12,12,12]).
 */
